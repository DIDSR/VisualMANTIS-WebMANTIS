...
void buffered_on_read(struct bufferevent *bev, void *arg) 
{
	...
	printf("Received %s from client %d. Strlen: %d, sizeof: %d\n", data, client->fd, strlen(data), sizeof(data));
	char * pch = NULL;
	int num;
	pch = strtok (data, " ");
	if(pch != NULL)
	{
		...
		if(num == 1 && (strcmp(pch, "QUERY") == 0))
		{
			system ("./deviceQuery > deviceQuery.txt");
		}
		else if(num == 2)
		{
			FILE *fp;
			...
			strcpy(processid, pch);
			strcat(processid, "/visualmantis/pid.txt");
			/* Open the command for reading. */
			fp = fopen(processid, "r");
			if (fp == NULL)
			{
		  	...
		  	while (fgets(path, sizeof(path)-1, fp) != NULL)
		  	{
				...
				strcat(str, path);
				int retd = system(str);
				//printf("return from kill %d\n", retd);
				
				if(retd == 0)
				{
					strcpy(sysecho, "echo 1 > ");
					strcat(sysecho, pch);
					strcat(sysecho, "/killed.txt");
					system(sysecho);
				}
				else
				{
					strcpy(sysecho, "echo 0 > ");
					strcat(sysecho, pch);
					strcat(sysecho, "/killed.txt");
					system(sysecho);
				}
		...
		else if(num == 3)
		{
			...
			strcpy(temp, "cd ");
			strcat(temp, pch);
			strcat(temp, "/visualmantis;./visualMANTIS_ver1_0.x < penEasy_CsI_input.in > visualmantis.output &");
			system(temp);
			
			char temp2[1000];
			strcpy(temp2, "cd ");
			strcat(temp2, pch);
			strcat(temp2, "/visualmantis;pidof visualMANTIS_ver1_0.x > pid.txt");
			system(temp2);
			//system("pidof visualMANTIS_ver1_0.x > pid.txt");
		}
	}
}
